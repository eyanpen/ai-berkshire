...
- description 包含：
  ...
  7. **金融严谨性验证（必须使用Bash调用工具，禁止心算）**：
     - 市值验算：`python3 "${CLAUDE_COMMANDS_DIR:-$HOME/.claude/commands}/tools/financial_rigor.py" verify-market-cap --price {价格} --shares {股本} --reported {报告市值} --currency {币种}`
     - 估值验算：`python3 "${CLAUDE_COMMANDS_DIR:-$HOME/.claude/commands}/tools/financial_rigor.py" verify-valuation --price {价格} --eps {EPS} --bvps {每股净资产}`
     - 关键数据交叉验证：`python3 "${CLAUDE_COMMANDS_DIR:-$HOME/.claude/commands}/tools/financial_rigor.py" cross-validate --field {字段} --values '{JSON}' --unit {单位}`
     - 三情景估值：`python3 "${CLAUDE_COMMANDS_DIR:-$HOME/.claude/commands}/tools/financial_rigor.py" three-scenario --price {价格} --eps {EPS} --shares {股本亿} --growth {乐观} {中性} {悲观} --pe {乐观PE} {中性PE} {悲观PE}`
...
python3 "${CLAUDE_COMMANDS_DIR:-$HOME/.claude/commands}/tools/report_audit.py" extract \
  --report <报告文件路径>

# Step 2 — 对清单每项从可靠信源取数（参见 skills/financial-data.md）

# Step 3 — 输出准出/打回判决
python3 "${CLAUDE_COMMANDS_DIR:-$HOME/.claude/commands}/tools/report_audit.py" verdict \
  --results '<填好的JSON>' \
  --report <报告文件名>
```
